 ...
 * Automated network synchronization between NetBox IPAM/NSX and and AVI.
 *
 * @example
 * ```typescript
 * const integrator: NAMAviIntegrator = {
 *   name: 'production-sync',
 *   sync_priority: 'high',
 *   enabled: true,
 *   address_family: '4',
 *   netbox_endpoint: new ObjectId('...'),
 *   avi_endpoints: [{ endpoint: {...}, vdoms: [...] }]
 * };
 * ```
 */
export interface NAMAviIntegrator extends NAMDefaultFields {
  /** Integrator name. */
  name: string;
  ...
  /** ALB endpoints for synchronization. */
  alb_endpoints: NAMAPIEndpoint[];
