...
/**
 * NAM v2 Vitistack Network Policy configuration.
 ...
 * @example
 * ```typescript
 * const networkPolicy: NAMVitiNetworkPolicy = {
 *   name: 'vitistack-network-policy1',
 *   enabled: true,
 *   desc: 'Vitistack network policy example',
 *   scope: 'internet',
 *   git_configs: [new ObjectId('...')],
...
export interface NAMVitiNetworkPolicy extends NAMDefaultFields {
  ...
  /** NSX VM tags for filtering. */
  vm_tags?: NetboxTag[] | ObjectId[];

  /** NSX group tags for filtering. */
  ...
  alb_endpoints: { endpoint: NAMAPIEndpoint }[];

  /** VMware NSX endpoints. */
  nsx_managers?: NAMAPIEndpoint[] | ObjectId[] | string[];
